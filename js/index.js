	...
	$.ajax({
		...
		success : function (response, status, xhr) {
			...
			$.each($('.editor'), function (index, value) {
				...
				summary[index] = arr[index].substr(0, 300);
				
				if (summary[index].substring(299,300) == '<') {
					summary[index] = replacePos(summary[index], 300, '');
				}
				if (summary[index].substring(298,300) == '</') {
					summary[index] = replacePos(summary[index], 300, '');
					summary[index] = replacePos(summary[index], 299, '');
				}
				
				if (arr[index].length > 300) {
